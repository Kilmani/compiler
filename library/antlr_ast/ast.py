...
import antlr4
from library.antlr_ast.inputstream import CaseTransformInputStream
...
class BaseAstVisitor(ParseTreeVisitor):
    ...
    def visitChildren(
        self, node: ParserRuleContext, predicate=None, simplify=True
    ) -> BaseNode:
    ...
    for field_name in field_names:
        ...
        if (
            ...
            if isinstance(field, list):
                ...
                # for el in field:
                #     print(str(ctx.children.index(el))+" index")
                #     print(str(el)+" el")
                #     print(field_name)'
                # # print(ctx.children.index(el))
                if str(value) != "[]" and str(field_name) != "eos":

                    field_dict[field_name] = value
                    # print(field_name)
                    # print(field_dict[field_name])
                    # print([print(el) for el in field])
